...
/// Top bar with app branding, notification bell, and logout button.
/// Fixed at the top of the viewport with a subtle drop shadow.
#[component]
        ...
        <header class="fixed top-0 left-0 right-0 z-40 bg-white shadow-sm">
            <div class="max-w-lg mx-auto flex items-center justify-between px-4 h-14">
                ...
                <A href="/" attr:class="flex items-center gap-2 min-h-[44px]" attr:aria-label="홈으로 이동">
                    ...
                    <span class="text-lg font-bold text-txt-primary">{t("nav.portal.senior")}</span>
                </A>
                ...
                <div class="flex items-center gap-1">
                    ...
                    <A
                        href="/notifications"
                        attr:class="relative flex items-center justify-center w-10 h-10 rounded-full hover:bg-surface-subtle transition-colors"
                        attr:aria-label=t("common.notifications")
                    >
                        <svg class="w-6 h-6 text-txt-secondary" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                        </svg>
                        <span class="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-danger rounded-full border border-white" />
                    </A>

                    // Logout button
                    <button
                        class="flex items-center justify-center w-10 h-10 rounded-full hover:bg-surface-subtle transition-colors"
                    ...
                    >
                        <svg class="w-5 h-5 text-txt-tertiary" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
/// Single bottom navigation tab with an icon and label.
/// Active state shows `text-primary` with a dot indicator; inactive uses `text-txt-disabled`.
#[component]
fn BottomNavItem(
    href: &'static str,
    label: &'static str,
    ...
    view! {
        <A
            href=href
            attr:class="flex-1 flex flex-col items-center py-2 gap-0.5 text-txt-disabled
                        [&.active]:text-primary"
        >
            ...
            <span class="hidden [.active_&]:block w-1 h-1 rounded-full bg-primary" />
        </A>
    }
}

/// Bottom navigation bar with five primary destinations.
/// Uses an upward box-shadow instead of a top border.
#[component]
...
/// Floating emergency call button, fixed at the bottom-right corner.
/// Uses a pulsing ring animation and danger-colored background for high visibility.
#[component]
    ...
    view! {
        <A
            ...
            attr:class="fixed bottom-24 right-4 z-50
                   ...
                   bg-danger hover:bg-red-700 active:bg-red-700
...
/// Sets `data-portal="senior"` on the root element for portal-scoped CSS variables.
#[component]
pub fn SeniorLayout(children: Children) -> impl IntoView {
    view! {
        <div class="text-scale-senior min-h-screen bg-surface-page" data-portal="senior">
            <TopBar />
            <main class="pt-14 pb-20 animate-fade-in" id="main-content">
                {children()}
            </main>
            <BottomNavBar />
            <EmergencyButton />
