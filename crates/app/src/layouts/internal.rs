...
/// A titled group of navigation items shown in the sidebar.
struct NavSection {
    title: &'static str,
...
/// Internal (provider/institution) portal layout.
///
/// Left sidebar (`w-60`) with 5 nav sections, top bar showing org name.
/// Uses `data-portal="internal"` to activate the `--portal-accent` /
/// `--portal-accent-light` CSS custom properties (`portal-internal` color).
/// Admin-focused compact layout with shadow-based section separation.
#[component]
pub fn InternalLayout(children: Children) -> impl IntoView {
    view! {
        <div data-portal="internal" class="flex h-screen bg-gray-50">
            // Sidebar
            <aside class="w-60 flex-shrink-0 bg-white shadow-[2px_0_8px_rgba(0,0,0,0.04)] flex flex-col">
                // Logo
                <div class="h-16 flex items-center px-4 border-b border-gray-100">
                    <div class="flex items-center gap-2">
                        <div class="w-8 h-8 bg-portal-internal rounded-lg flex items-center justify-center">
                            <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                            </svg>
                        </div>
                        <div>
                            <p class="text-sm font-bold text-gray-900">{t("nav.portal.internal")}</p>
                            <p class="text-xs text-gray-500">{t("nav.internal_admin")}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-4 border-t border-gray-100">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 bg-[var(--portal-accent-light)] rounded-full flex items-center justify-center">
                            <span class="text-xs font-semibold text-[var(--portal-accent)]">"관"</span>
                        </div>
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
                ...
                <header class="h-16 bg-white shadow-sm flex items-center justify-between px-6 flex-shrink-0">
                    ...
                    <div class="flex items-center gap-3">
                        <a
                            href="/internal/referrals"
                            class="relative p-2 text-[var(--portal-accent)] hover:text-gray-700 hover:bg-[var(--portal-accent-light)] rounded-lg transition-colors"
                        >
