...
/// Shows a single payment detail view, fetched from benefits/utilization API.
/// Re-fetches the list and filters by the id from the URL.
#[component]
pub fn PaymentDetailPage() -> impl IntoView {
    ...
    let data = LocalResource::new(move || {
        ...
        async move {
            let resp = crate::api::get::<serde_json::Value>("/api/benefits/utilization").await;
            match resp {
                Ok(api_resp) if api_resp.success => {
                    let payload = api_resp.data.unwrap_or(serde_json::Value::Null);
                    let items: Vec<serde_json::Value> = if let Some(arr) = payload.as_array() {
                        arr.clone()
                    } else if payload.is_object() {
                        vec![payload]
                    ...
                    // Find by id field or by index
                    items.into_iter().enumerate().find(|(i, item)| {
                ...
                {move || Suspend::new(async move {
                    match data.await {
                        Some(item) => {
                            ...
                            let service = item.get("service_name")
                                .or_else(|| item.get("program_name"))
                                .or_else(|| item.get("label"))
                                .and_then(|v| v.as_str())
                                .unwrap_or("결제 항목")
                            ...
                            let amount = item.get("amount")
                                ...
                                .and_then(|v: &serde_json::Value| v.as_str().map(|s| s.to_string())
                                    .or_else(|| v.as_f64().map(|n| format!("₩{:.0}", n))))
                                .unwrap_or_else(|| "-".to_string());
                            let copay = item.get("copay")
                                .or_else(|| item.get("self_pay"))
                            ...
                            view! {
                                <div class="bg-surface-card rounded-2xl p-5 shadow-sm space-y-4">
                                    <div class="flex justify-between">
                                        ...
                                        <p class="font-medium text-txt-primary">{service}</p>
                                    </div>
                                    <div class="flex justify-between">
                                        <p class="text-sm text-txt-tertiary">"기간"</p>
                                        <p class="text-sm text-txt-secondary">{period}</p>
                                    </div>
                                    <div class="flex justify-between">
                                    ...
                                    <div class="flex justify-between">
                                        <p class="text-sm text-txt-tertiary">"본인부담금"</p>
                                        <p class="font-bold text-[var(--portal-accent)]">{copay}</p>
                                    </div>
                                    <div class="flex justify-between">
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <p class="text-sm text-txt-secondary">{paid_at}</p>
                                    </div>
                                </div>
                            }.into_any()
                        }
                        _ => view! {
                            <div class="bg-surface-card rounded-2xl p-5 shadow-sm">
                                <p class="text-sm text-txt-tertiary">"결제 정보를 불러올 수 없습니다."</p>
                            </div>
                        }.into_any(),
                    }
                })}
