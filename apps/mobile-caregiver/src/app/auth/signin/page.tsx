...
const APP_NAME = '요양보호사 앱';

const DEMO_USERS = [
  {
    name: '박미영',
    email: 'miyoung.park@example.kr',
    role: 'CAREGIVER_APPLICANT',
    roleLabel: '지원자',
    description: '요양보호사 지원 중',
  },
  {
    name: '이지은',
    email: 'jieun.lee@example.kr',
    role: 'CAREGIVER_APPROVED',
    roleLabel: '승인 완료',
    description: '현직 요양보호사',
  },
] as const;
...
const THEME = {
  ...
  badge: {
    CAREGIVER_APPLICANT: 'bg-amber-100 text-amber-700',
    CAREGIVER_APPROVED: 'bg-green-100 text-green-700',
  },
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className={`bg-gradient-to-br ${THEME.gradient} rounded-2xl p-6 text-white text-center mb-8`}>
          ...
          <p className="text-blue-200 mt-1 text-sm">데모 로그인</p>
        </div>
        ...
        <div className="space-y-3">
          {DEMO_USERS.map((user) => {
            ...
            const badgeClass = THEME.badge[user.role as keyof typeof THEME.badge];
            return (
              <button
                key={user.email}
                type="button"
