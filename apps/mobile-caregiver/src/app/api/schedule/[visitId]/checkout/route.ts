...
import { VisitCheckOutSchema } from '@bominal-senior/types';
import { checkOut } from '@/lib/services';
...
export async function POST(
  ...
  try {
    ...
    const parsed = VisitCheckOutSchema.safeParse(body);
    ...
    if (visitResult.rows[0].caregiver_id !== appResult.rows[0].id) {
      return apiError('Not authorized to check out from this visit', 403);
    }

    const updated = await checkOut(pool, visitId, {
    ...
    console.error('[POST /api/schedule/[visitId]/checkout]', error);
    if (error instanceof Error) {
      return apiError(error.message, 400);
    }
    return apiError('Failed to check out from visit', 500);
  }
}
